...
def base_stats(df, array):
    ...
    ----------
    Параметры:
            df - исохдный датафрейм
            array - названия колонок для базовой статистики
    ----------
    Возвращает: датафрейм базовой статистики
    ----------
    Автор: Чихватова А.А.
    """
    bst = ["Количество элементов", "Уникальные элементы",
           "Самый частый элемент", "Количество повторений",
           "Среднее значение", "Среднеквадратическое отклонение",
           "Минимальный элемент", "25%", "50%",
           "75%", "Максимальный элемент"]
    df3 = pd.DataFrame(bst, columns=['Свойства'], index=['count', 'unique', 'top', 'freq', 'mean',
